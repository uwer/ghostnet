GetClusterPredictions <- function(MeshSize,TwineSize,Colors,Construction){
...
#
#Code is now modified where it expects a single observation's worth of data, ie 1 value for each of the
#4 net characteristics.  It then returns 3 things appended together in a vector.  The first entry is the index
#of the net type (ie 1 = type 1), the second entry is the sum of the probabilities across all 14 net types,
#which gives a measure of the uncertainty in assignment, and the third is 14 probabilities one for each
#possible net type.
...
TypeProbs <- vector(length = 14)
 ...
 index <- match(c(Colors,Construction),colnames(DiscreteProbs))
 TypeProbs<- DiscreteProbs[,index[1]]/sum(DiscreteProbs[,index[1]]) * DiscreteProbs[,index[2]]/sum(DiscreteProbs[,index[2]]) * SizeProbs

#get the most likely net type index
MostLikelyType <- which.max(TypeProbs)

#get the sum of the probabilities across the types, this is to some extent a measure of the quality of the assignment
TotalProbs <- sum(TypeProbs)

#bind together the data, the type probabilities, and the total probability across types
AssignmentInfo <- c(MostLikelyType ,TotalProbs,TypeProbs)
